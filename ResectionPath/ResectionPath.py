  ...
  def recolorLabelMap(self, modelNode, labelMap, initialValue, outputValue):
    if (modelNode != None and labelMap != None):
      ...
      for point in range(numberOfPoints):
      	if (self.filter.IsInside(point)):
      		z = point/(x_dim * y_dim)
      		y = (point - z*(x_dim * y_dim))/x_dim
      		x = point % x_dim
      		if (self.labelMapImgData.GetScalarComponentAsDouble(x,y,z,0) == initialValue):
      		  self.labelMapImgData.SetScalarComponentFromDouble(x,y,z,0,outputValue)
